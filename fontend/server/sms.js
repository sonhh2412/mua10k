...
mongoose.connect('mongodb://admin10kmua:Admin10kmua#$%@127.0.0.1:27017/10k_product');
                ...
                User.findOne({
                    ...
                    if(!err && _.size(user) > 0){
                        ...
                        if(totalAmount <= 0){
                            ...
                            var k_number = totalAmount / 1000 * 0.6;
                            ...
                            User.update ( {
                                ...
                                !err ? ( 
                                    ControlUser.updateNotifyMess([{ _id: user._id }], 'my_wallet', function(result){
                                        ...
                                        Notification10k.sendNotificationforOneUser('Bạn vừa nạp'+(k_number).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.")+' đ từ số điện thoại '+request.post.isdn.replace('84','0')+' ! ', user._id)
                                    }),
                                    data_push = {
                                    ...
                                    new websiteTransactionK_error(data_push).save(function(err, result){
                                        !err ? ( 
                                            callback(true), 
                                            console.log("Seccess user: "+ request.post.account+" total " + parseInt(request.post.totalAmount).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") + "đ sdt: " + request.post.isdn.replace('84','0')) )
                                        : callback(false);
                                    })
...
app.listen(9972, function(){
    console.log('Notify SMS running on port 9972');
})
